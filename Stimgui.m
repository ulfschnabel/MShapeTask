...
function T_Shape_Callback(hObject, eventdata, handles)
% hObject    handle to T_Poly (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    item = get(handles.StimObjLB,'Value');
       ...
       elseif strcmp(OBJ(item).Type, 'Polyline')
        ...
        elseif strcmp(OBJ(item).Type, 'Shape')
            Shape = OBJ(item).Data;
            Id = OBJ(item).Id;
            StimObj.Obj(item).Data = cgShape(Shape, 'E' , Id);
        end
    ...
    cgflip(BGcolor(1), BGcolor(2), BGcolor(3));
           ...
           ObjIds = [ Objs(:).Id ];
            ...
            for i = 1:length(Ids)
               ...
               elseif strcmp(Objs(Oix).Type, 'Shape')
                   Objs(Oix).Data = cgShape(Objs(Oix).Data, 'sprite', Ids(i)); %load the Randompattern
               end                 
            end
